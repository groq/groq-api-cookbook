...
def get_verified_sql(embedding_model,user_question,verified_queries_dict,minimum_similarity):
    ...
    if highest_similarity >= minimum_similarity / 100.0:
        ...
        print(f"Found a verified query: **{most_similar_prompt}** ({round(highest_similarity*100,1)}% similarity)")
        #st.markdown("```sql\n" + sql_query + "\n```")
        return sql_query
    else:
...
def main():
    ...
    gets user input from the command line, retrieves and executes the most similar verified SQL query, and displays
    ...
    minimum_similarity = 0 #Increase this to make the LLM more picky
    ...
    while True:
        ...
        verified_sql_query = get_verified_sql(embedding_model,user_input,verified_queries_dict,minimum_similarity)
        ...
        if verified_sql_query is not None:
            ...
            summarization = get_summarization(client,user_input,results_df,model)
